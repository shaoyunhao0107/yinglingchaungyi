# -*- coding: utf-8 -*-
path = r'G:\AI\monica\monica-proxy-master\internal\apiserver\anthropic.go'

code = r"""package apiserver
...
func anthropicToChatRequest(r *AnthropicRequest) (*openai.ChatCompletionRequest, error) {
	...
	if r.System != nil {
		if sysText := anthropicContentToString(r.System); sysText != "" {
		...
		if json.Unmarshal(jsonBytes, &oaiChunk) == nil && len(oaiChunk.Choices) > 0 {
			...
			for _, tc := range choice.Delta.ToolCalls {
				...
				if tc.Function.Arguments != "" {
					...
					if err := writeAnthropicSSE(w, "content_block_delta", 