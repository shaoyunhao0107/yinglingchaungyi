...
type SSEData struct {
	...
	Error       *ErrorInfo  `json:"error,omitempty"` // 新增错误字段
}
...
func CollectMonicaSSEToCompletion(model string, r io.Reader) (*openai.ChatCompletionResponse, error) {
	...
	err := processor.processSSEStream(func(sseData *SSEData) error {
		...
		// Monica 返回错误时，作为 error 返回而非写入内容
		if sseData.Error != nil && sseData.Error.Msg != "" {
			return fmt.Errorf("monica error %d: %s", sseData.Error.Code, sseData.Error.Msg)
		}
		// 累积正常内容
...
func StreamMonicaSSEToClient(model string, w io.Writer, r io.Reader) error {
	...
	return processor.processSSEStream(func(sseData *SSEData) error {
		...
		if sseData.Error != nil && sseData.Error.Msg != "" {
			...
			sb.WriteString("data: ")
			textLine, _ := sonic.MarshalString(sseMsg)
		...
		case sseData.Finished:
			// finished=true 时，Text 里可能还有最后一段内容，先把它发出去
			if sseData.Text != "" {
				if thinkFlag {
					sseData.Text = "</think>" + sseData.Text
					thinkFlag = false
				}
				textMsg := types.ChatCompletionStreamResponse{
					ID:                "chatcmpl-" + chatId,
					Object:            sseObject,
					SystemFingerprint: fingerprint,
					Created:           now,
				...
				sb2 := stringBuilderPool.Get().(*strings.Builder)
				sb2.WriteString("data: ")
				textLine, _ := sonic.MarshalString(textMsg)
				sb2.WriteString(textLine)
				sb2.WriteString("\n\n")
				writer.WriteString(sb2.String())
				sb2.Reset()
				stringBuilderPool.Put(sb2)
			}
			sseMsg = types.ChatCompletionStreamResponse{
				ID:      "chatcmpl-" + chatId,
				Object:  sseObject,
