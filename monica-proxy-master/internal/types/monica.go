...
// modelToBotMap maps an OpenAI-style model name to Monica's internal bot_uid.
// The 23 models below are the full current Monica lineup, each verified LIVE
// against Monica (probed 2026-06-25). Monica retires bot_uids frequently; re-probe
// with _probe_models.py and prune any that start returning "已下线" / "discontinued".
// use_model must stay empty (see ChatGPTToMonica) or newer models return
...
var modelToBotMap = map[string]string{
	...
	"claude-4-7-opus":   "claude_4_7_opus",
...
func ChatGPTToMonica(cfg *config.Config, chatReq openai.ChatCompletionRequest) (*MonicaRequest, error) {
	...
	mReq := &MonicaRequest{
		...
		Data: DataField{
			...
			TriggerBy:       "auto",
			IsIncognito:     true,
			// use_model 必须留空：模型由 bot_uid 决定。较新的模型（claude-4-7-opus /
			// gemini-3.1-pro / grok-4.3 等）在 use_model 非空时会返回 "Internal server error"。
			UseModel:     "",
			UseNewMemory: false,
		},
		Language: "auto",
