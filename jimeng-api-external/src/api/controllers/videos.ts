...
function getVideoBenefitType(model: string): string {
  ...
  // ── Seedance 40 家族（2026 新一代）──
  // 抓包确认 Fast VIP (dreamina_seedance_40_vision): benefit_type="dreamina_seedance_20_fast_5s"
  if (model.includes("40_vision")) {
    return "dreamina_seedance_20_fast_5s";
  }
  // 抓包确认 VIP (dreamina_seedance_40_pro_vision): benefit_type="seedance_20_pro_720p_output"
  if (model.includes("40_pro_vision")) {
    return "seedance_20_pro_720p_output";
  }
  // mini (dreamina_seedance_40_mini)：抓包确认 benefit_type="seedance_20_mini_720p_output"
  if (model.includes("40_mini")) {
    return "seedance_20_mini_720p_output";
  }
...
export async function generateVideo(
  ...
  // 如果无法获取视频URL，检查是否是审核失败
  if (!fallbackVideoUrl) {
    // 审核失败检测
    const fc = (finalHistoryData as any)?.fail_code;
    const fstat = pollingResult.status;
    if (fstat === 30 || fc) {
      logger.error(`视频生成失败: status=${fstat}, fail_code=${fc}`);
      throw new APIException(EX.API_VIDEO_GENERATION_FAILED,
        `视频生成失败${fc ? `（错误码: ${fc}）` : ''}：视频未通过审核或被拒绝。请修改提示词或素材后重试。`);
    }
