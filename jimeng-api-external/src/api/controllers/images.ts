...
export async function generateImageComposition(
  ...
  // 审核失败检测
  const _fs1 = pollingResult.status;
  if (resultImageUrls.length === 0) {
    const fc = (finalTaskInfo as any)?.fail_code;
    if (_fs1 === 30 || fc) {
      throw new APIException(EX.API_IMAGE_GENERATION_FAILED,
        `图生图失败${fc ? `（错误码: ${fc}）` : ''}：图片未通过审核或被拒绝。请修改提示词或素材后重试。`);
    }
    throw new APIException(EX.API_IMAGE_GENERATION_FAILED,
      `图生图失败：未返回任何结果（状态码: ${_fs1}）`);
  }
...
async function generateImagesInternal(
  ...
  // ── 审核失败 / 生成失败检测 ──
  // 即梦审核不通过时 item_list 为空且 polling status=30，但 handleGenerationFailure
  // 可能因为 poller 内部逻辑没正确抛出。这里加一层兜底。
  const finalStatus = pollingResult.status;
  if (imageUrls.length === 0) {
    const failCode = (finalTaskInfo as any)?.fail_code;
    if (finalStatus === 30 || failCode) {
      const reason = failCode ? `（错误码: ${failCode}）` : '';
      // 常见 fail_code: 审核不通过、内容违规、敏感词
      throw new APIException(EX.API_IMAGE_GENERATION_FAILED,
        `图像生成失败${reason}：图片未通过审核或被拒绝。请修改提示词后重试。`);
    }
    throw new APIException(EX.API_IMAGE_GENERATION_FAILED,
      `图像生成失败：未返回任何结果（状态码: ${finalStatus}）`);
  }
...
async function generateJimeng4xMultiImages(
  ...
  // 审核失败检测
  const _fs3 = pollingResult.status;
  if (imageUrls.length === 0) {
    const fc = (finalTaskInfo as any)?.fail_code;
    if (_fs3 === 30 || fc) {
      throw new APIException(EX.API_IMAGE_GENERATION_FAILED,
        `多图生成失败${fc ? `（错误码: ${fc}）` : ''}：图片未通过审核或被拒绝。请修改提示词后重试。`);
    }
    throw new APIException(EX.API_IMAGE_GENERATION_FAILED,
      `多图生成失败：未返回任何结果（状态码: ${_fs3}）`);
  }
